...
<head>
	...
	tr{height:100px;width:100%;background:red}
	body {
...
<table border="0">
	...
	if($corrections_travaux1){?>
			...
			<?php if($corrections_travaux1){foreach($corrections_travaux1 as $correction){
				...
				<tr>
				<td align="left" style="list-style:square;color:red"><b><?php echo $correction->localisation;?></b> - <?php echo $correction->intitile;?> (<?php echo $etat;?>) <span style="color:red"><?php if($correction->etat_urgent==2) echo '<span class="label bg-red shadow-style">Urgent</span>';?></span>
	...
	<?php }
	$corrections_travaux2=$this->md_commun->fetch_creat('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'appartements_id'=>$app->id,'travaux_id'=>$trav->id,'etat'=>1,'etat_urgent'=>1,'etat_correction !='=>3),'asc',100,0);
	//correction travaux
	if($corrections_travaux2){?>
			...
			<?php if($corrections_travaux2){foreach($corrections_travaux2 as $correction){
				...
				if($correction->etat_correction==1){$etat='Crée';}elseif($correction->etat_correction==4){$etat='En cours';}elseif($correction->etat_correction==2){$etat='Corrigée';}elseif($correction->etat_correction==3){$etat='Validé';}?>
				<tr>
				<td align="left" style="list-style:square;float:left;width:100%;"><b><?php echo $correction->localisation;?></b> - <?php echo $correction->intitile;?> (<?php echo $etat;?>) <span style="color:red"><?php if($correction->etat_urgent==2) echo '<span class="label bg-red shadow-style">Urgent</span>';?></span>
				<?php if($historiques){foreach($historiques as $hit){echo '<br><span style="color:red;text-decoration:underline;margin-left-left:10px;">'.$hit->observation_leve.'</span>'; }}?>
				</td>
				</tr>
			<?php }}?>
...
if($corrections_app){?>
	...
	if($corrections_travaux1){?>
			...
			<?php if($corrections_travaux1){foreach($corrections_travaux1 as $correction){ 
				...
				<td align="left" style="width:220px;padding-right:10px;float:left; padding-bottom:10px;">
					<img src="<?php echo uploads('images/full/'.$correction->image);?>" style="height:220px;object-fit:cover">
					<div align="left" style="border:solid 1px #585499;width:100%;float:left;background:#e9e9e9;height:30px;font-size:10px;">
	...
	if($corrections_travaux2){?>
			<?php if($corrections_travaux2){foreach($corrections_travaux2 as $correction){
				...
				<td align="left" style="width:220px;padding-right:10px;float:left; padding-bottom:10px">
					<img src="<?php echo uploads('images/full/'.$correction->image);?>" style="float:left;height:220px;object-fit:cover">
					<div style="border:solid 1px #585499;background:#e9e9e9;height:30px;font-size:10px;">
						<b>[<?php echo $app->appartements;?>] <?php echo $correction->localisation;?></b> - <?php echo $correction->intitile;?> (<?php echo $etat;?>) <span style="color:red"><?php if($correction->etat_urgent==2) echo '<span class="label bg-red shadow-style">Urgent</span>';?></span>
					</div>	
				</td>
				<?php if($ic%3==0) echo '</tr><tr>';?>
