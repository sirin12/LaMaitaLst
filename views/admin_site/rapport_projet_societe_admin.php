...
$corrections_bloc=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etat'=>1,'afficher'=>1),'asc',1000,0);
//correction bloc
if($corrections_bloc){?>
<?php $etages=$this->md_commun->fetch('etages',array('projet_id'=>$projet_id,'blocs_id'=>$bloc->id,'etat'=>1),'asc',1000,0);?>
<?php if($etages){foreach($etages as $etage){
$corrections_etage=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'etat'=>1,'afficher'=>1),'asc',1000,0);
//correction etage
if($corrections_etage){?>
<?php $apps=$this->md_commun->fetch('appartements',array('projet_id'=>$projet_id,'blocs_id'=>$bloc->id,'etages_id'=>$etage->id,'etat'=>1),'asc',1000,0);?>
<?php if($apps){foreach($apps as $app){
$corrections_app=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'appartements_id'=>$app->id,'etat'=>1,'afficher'=>1),'asc',1000,0);
//correction etage
if($corrections_app){?>
...
<?php $current_admin = $this->session->userdata('admin');
	...
	<?php if($travaux){foreach($travaux as $trav){
		$travau_sos=$this->md_commun->get_row('travaux_societe',array('projet_id'=>$projet_id,'societe_id'=>$societe_id,'travaux_id'=>$trav->id));
		if($travau_sos){
	...
	<tr><th align="left"><b><u><?php echo $bloc->blocs;?></u></b><br></th></tr>
	<tr><th align="left"><b><u>* <?php echo $etage->etages;?></u></b><br></th></tr>
	<tr><th align="left">- <?php echo $app->appartements.$societe_id;?></th></tr>
	<tr>
		<td align="left">
			...
			<?php if($corrections_travaux1){foreach($corrections_travaux1 as $correction){
				...
				<div style="list-style:square;color:red"><b><?php echo $correction->localisation;?></b> - <?php echo $correction->intitile;?> (<?php echo $etat;?>) <span style="color:red"><?php if($correction->etat_urgent==2) echo '<span class="label bg-red shadow-style">Urgent</span>';?></span>
	...
	<?php }
	$corrections_travaux2=$this->md_commun->fetch_creat('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'appartements_id'=>$app->id,'travaux_id'=>$trav->id,'etat'=>1,'afficher'=>1,'etat_urgent'=>1,'etat_correction !='=>3),'asc',100,0);
	//correction travaux
	...
	<tr>
		<td align="left">
			...
			<?php if($corrections_travaux2){foreach($corrections_travaux2 as $correction){
				$historiques=$this->md_commun->fetch('corrections_historiques',array('correction_id'=>$correction->id,'etat'=>1),'asc',100,0);
				if($correction->etat_correction==1){$etat='Crée';}elseif($correction->etat_correction==4){$etat='En cours';}elseif($correction->etat_correction==2){$etat='Corrigée';}elseif($correction->etat_correction==3){$etat='Validé';}?>
				<div style="list-style:square;float:left;width:100%;margin-bottom:50px;height:50px;line-height:2"><b><?php echo $correction->localisation;?></b> - <?php echo $correction->intitile;?> (<?php echo $etat;?>) <span style="color:red"><?php if($correction->etat_urgent==2) echo '<span class="label bg-red shadow-style">Urgent</span>';?></span>
				<?php if($historiques){foreach($historiques as $hit){echo '<br><span style="color:red;text-decoration:underline;margin-left-left:10px;">'.$hit->observation_leve.'</span>'; }}?>
				</div>
			<?php }}?>
		</td>
	...
	<tr>
		<td align="left">
			...
			<?php if($corrections_travaux3){foreach($corrections_travaux3 as $correction){
				$historiques=$this->md_commun->fetch('corrections_historiques',array('correction_id'=>$correction->id,'etat'=>1),'asc',100,0);
				if($correction->etat_correction==1){$etat='Crée';}elseif($correction->etat_correction==4){$etat='En cours';}elseif($correction->etat_correction==2){$etat='Corrigée';}elseif($correction->etat_correction==3){$etat='Validé';}?>
				<div style="list-style:square;color:blue;"><b><?php echo $correction->localisation;?></b> - <?php echo $correction->intitile;?> (<?php echo $etat;?>) 
