...
$corrections_bloc=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etat'=>1,'afficher'=>1,'afficher'=>1,'etat_correction'=>2),'asc',100,0);
//correction bloc
if($corrections_bloc){?>
...
$corrections_etage=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'etat'=>1,'afficher'=>1,'afficher'=>1,'etat_correction'=>2),'asc',100,0);
//correction etage
...
$corrections_app=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'appartements_id'=>$app->id,'etat'=>1,'afficher'=>1,'afficher'=>1,'etat_correction !='=>3),'asc',100,0);
//correction app
if($corrections_app){?>
<table>
	<thead>
	...
	$corrections_travaux=$corrections=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'appartements_id'=>$app->id,'travaux_id'=>$trav->id,'etat'=>1,'afficher'=>1,'afficher'=>1,'etat_correction'=>2),'asc',100,0);
	//correction travaux
	...
	<tr>
		<td align="left">
			...
			<?php if($corrections){foreach($corrections as $correction){
				...
				<li style="list-style:square"><b><?php echo $correction->localisation;?></b> - <?php echo $correction->intitile;?> (<?php echo $etat;?>) <span style="color:red"><?php if($correction->etat_urgent==2) echo '<span class="label bg-red shadow-style">Urgent</span>';?></span></li>
			<?php }}?>
		</td>
	</tr>
	<?php }?>
...
ob_end_clean();
		...
		$mpdf->Output('rapport_corriger.pdf', 'I');
		?>