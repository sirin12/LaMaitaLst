                            ...
                            <div class="table-responsive">
                                <table id="tableExport"
									...
									<?php $current_admin = $this->session->userdata('admin');
										...
										if($rapports){foreach($rapports as $rapport){?>
                                        <tr>
											...
											<td width="390">
												...
												<a href="<?php echo admin_url('Rapport_societe/delete/'.$rapport->id);?>" class="btn bg-red waves-effect" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fas fa-trash-alt"></i> <span class="btn_desctopp" >Supprimer</span></a>
											</td>
                                        </tr>
                                    <?php }}
										}else{
										$projets_id=$this->md_commun->fetch('projets_societe_admin',array('admin_id'=>$id_admin),'asc',10000,0);
										if($projets_id){foreach($projets_id as $projad){
										$rapportsproj=$this->md_commun->fetch('rapport_projet_societe',array('projet_id'=>$projad->projet_id,'etat'=>1),'asc',10000,0);
										if($rapportsproj){foreach($rapportsproj as $rapport){?>
                                        <tr>
											<td><?php echo $rapport->sujet;?></td>
                                            <td><?php echo $rapport->date_time;?></td>
											<td width="390">
