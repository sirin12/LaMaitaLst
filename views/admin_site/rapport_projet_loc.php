...
$corrections_bloc=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc_id,'etat'=>1,'afficher'=>1),'asc',100,0);
//correction bloc
if($corrections_bloc){?>
<b><u><?php echo $bloc->blocs;?></u></b><br>
<?php $etages=$this->md_commun->fetch('etages',array('projet_id'=>$projet_id,'blocs_id'=>$bloc_id,'id'=>$etage_id,'etat'=>1),'asc',100,0);?>
<?php if($etages){foreach($etages as $etage){
$corrections_etage=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc_id,'etage_id'=>$etage_id,'appartements_id'=>$loc_id,'etat'=>1,'afficher'=>1),'asc',100,0);
//correction etage
if($corrections_etage){?>
<b><u>* <?php echo $etage->etages;?></u></b><br>
<?php $apps=$this->md_commun->fetch('appartements',array('projet_id'=>$projet_id,'blocs_id'=>$bloc->id,'etages_id'=>$etage->id,'id'=>$loc_id,'etat'=>1),'asc',100,0);?>
<?php if($apps){foreach($apps as $app){
$corrections_app=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'appartements_id'=>$loc_id,'etat'=>1,'afficher'=>1),'asc',100,0);
//correction etage
if($corrections_app){?>
<table>
	<thead>
	...
	$corrections_travaux1=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc_id,'etage_id'=>$etage_id,'appartements_id'=>$loc_id,'travaux_id'=>$trav->id,'etat'=>1,'afficher'=>1,'etat_urgent'=>2,'etat_correction !='=>3),'asc',100,0);
	//correction travaux
	...
	<?php }
	$corrections_travaux2=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc_id,'etage_id'=>$etage_id,'appartements_id'=>$loc_id,'travaux_id'=>$trav->id,'etat'=>1,'afficher'=>1,'etat_urgent'=>1,'etat_correction !='=>3),'asc',100,0);
	//correction travaux
	...
	<?php }
	$corrections_travaux3=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc_id,'etage_id'=>$etage_id,'appartements_id'=>$loc_id,'travaux_id'=>$trav->id,'etat'=>1,'afficher'=>1,'etat_correction'=>3),'asc',100,0);
	//correction travaux
...
<?php if($blocs){foreach($blocs as $bloc){
$corrections_bloc=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc_id,'etat'=>1,'afficher'=>1,'etat_correction !='=>3),'asc',100,0);
//correction bloc
if($corrections_bloc){?>
<?php $etages=$this->md_commun->fetch('etages',array('projet_id'=>$projet_id,'blocs_id'=>$bloc_id,'id'=>$etage_id,'etat'=>1),'asc',100,0);?>
<?php if($etages){foreach($etages as $etage){
$corrections_etage=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc_id,'etage_id'=>$etage_id,'appartements_id'=>$loc_id,'etat'=>1,'afficher'=>1,'etat_correction !='=>3),'asc',100,0);
//correction etage
if($corrections_etage){?>
<?php $apps=$this->md_commun->fetch('appartements',array('projet_id'=>$projet_id,'blocs_id'=>$bloc->id,'etages_id'=>$etage->id,'id'=>$loc_id,'etat'=>1),'asc',100,0);?>
<?php if($apps){foreach($apps as $app){
$corrections_app=$this->md_commun->fetch('corrections',array('projet_id'=>$projet_id,'bloc_id'=>$bloc->id,'etage_id'=>$etage->id,'appartements_id'=>$loc_id,'etat'=>1,'afficher'=>1,'etat_correction !='=>3),'asc',100,0);
//correction etage
if($corrections_app){?>
										<?php $current_admin = $this->session->userdata('admin');
		$id_admin=$current_admin['id']; if ($current_admin['access'] == 'Admin') {
