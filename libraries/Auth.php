...
class Auth {
    ...
    function is_logged_in($redirect = false, $default_redirect = true) {
        ...
        if (!$admin) {
            ...
            if (isset($_COOKIE['smartAdmin'])) {

                //the cookie is there, lets log the customer back in.

                $info = $this->aes256Decrypt(base64_decode($_COOKIE['smartAdmin']));
    ...
    function login_admin($username, $password, $remember = false) {
        ...
        if (sizeof($result) > 0) {
			...
			$admin['admin']['super_admin'] = $result['super_admin'];
			
			$admin['admin']['type_admin'] = $result['type_admin'];
			
			$admin['admin']['client_id'] = $result['client_id'];


            if ($remember) {

                $loginCred = json_encode(array('email' => $username, 'password' => $password));
    ...
    private function generateCookie($data, $expire) {
        ...
        setcookie('smartAdmin', $data, $expire, '/', $_SERVER['HTTP_HOST']);
    }
