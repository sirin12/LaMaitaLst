...
function rtf2text($text) {
    ...
    $document = preg_replace("/MS Sans Serif;|Verdana;|;;/", "", $document);
    // Return result.
    return utf8_encode($document);
}
...
function setTextBetweenTags($string, $replace, $tagname) {
    $pattern = "/<$tagname>([\w\W]*?)<\/$tagname>/";
    return preg_replace($pattern, '<' . $tagname . '>' . $replace . '</' . $tagname . '>', $string);
}
...
function has_htmltag($string, $tagname) {
    $pattern = "/<$tagname>([\w\W]*?)<\/$tagname>/";
    return preg_match($pattern, $string, $m) != 0;
}

function Clang($string)
{
	$ci=& get_instance();
	$term=json_decode($string);
	if(is_object($term))
	return $term->{$ci->current_lang->flag};
	else
	return $string;
}
?>