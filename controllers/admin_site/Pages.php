...
class Pages extends Admin_Controller {
	...
	function upload_correction()	{
        ...
        $this->load->library('upload', $config);
		/*$uploaded = $this->upload->do_upload($file);*/
        $upload_imgs=$this->md_commun->fetch('corrections',array(),'asc',10000,0);
		if ($upload_imgs){foreach($upload_imgs as $upload_img){
			
			if($upload_img->image){ echo '<br>id='.$upload_img->id;
                $image = $this->upload->data();
                $this->load->library('image_lib');
                //this is the larger image
                $config['image_library'] = 'gd2';
                $config['source_image'] = 'uploads/images/full/' . $upload_img->image;
                $config['new_image'] = 'uploads/images/medium/' . $upload_img->image;;
                $config['maintain_ratio'] = TRUE;
                $config['width'] = 500;
                $config['height'] = 500;
                $this->image_lib->initialize($config);
                ...
                //small image
                $config['image_library'] = 'gd2';
                $config['source_image'] = 'uploads/images/medium/' . $upload_img->image;
                $config['new_image'] = 'uploads/images/small/' . $upload_img->image;
                $config['maintain_ratio'] = TRUE;
                $config['width'] = 200;
                $config['height'] = 200;
                $this->image_lib->initialize($config);
