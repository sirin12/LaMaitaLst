<?php
class Enseignants_class extends Admin_Controller {

    function __construct() {
        parent::__construct();

        $this->auth->check_access('Admin', true);
		$this->load->model(array('Coefficient_model','Enseignants_model','Class_model','Filieres_model','Matieres_model'));
    }

    function index() {
        $data['page_title'] = 'Enseignants class';
        $data['coefficients'] = $this->Coefficient_model->get();
        $data['page_icon'] = 'icon-file-alt';

        $this->view('enseignants_class', $data);
    }
