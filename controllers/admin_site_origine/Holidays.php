<?php
class Holidays extends Admin_Controller {

    function __construct() {
        parent::__construct();

        $this->auth->check_access('Admin', true);
		$this->load->model('Holidays_model');
    }

    function index() {
        $data['page_title'] = 'holidays';
        $data['holidays'] = $this->Holidays_model->get();
        $data['page_icon'] = 'icon-file-alt';

        $this->view('holidays', $data);
    }

    /*     * ******************************************************************
      edit categorie
     * ****************************************************************** */

    function form($id = false) {

        $this->load->helper('url');
        $this->load->helper('form');
        $this->load->library('form_validation');

        //set the default values
        $data['id'] = '';
		$data['name'] = '';
		$data['date_debut'] = '';
		$data['date_fin'] = '';
		
        $data['page_title'] = 'holidays';
        $data['page_icon'] = 'icon-file-alt';
        if ($id) {

            $holiday = $this->Holidays_model->get($id);
				
            if (!$holiday) {
                //page does not exist
                $this->session->set_flashdata('error', lang('error_page_not_found'));
                redirect($this->config->item('admin_folder') . '/holidays');
            }

            //set values to db values
            $data['id'] = $holiday->id;
			$data['name'] = $holiday->name;
			$data['date_debut'] = $holiday->date_debut;
			$data['date_fin'] = $holiday->date_fin;
        }

        $this->form_validation->set_rules('name', 'name', 'trim');

        // Validate the form
        if ($this->form_validation->run() == true) {
            $this->load->helper('text');

			$save = array();
			$save['id'] = $id;
			$save['name'] = $this->input->post('name');
			$save['date_debut'] = DateToMysqlFormat($this->input->post('date_debut'));
			$save['date_fin'] = DateToMysqlFormat($this->input->post('date_fin'));
			$holidays_id = $this->Holidays_model->save($save);
			$this->session->set_flashdata('message', lang('message_saved_categorie'));
			redirect($this->config->item('admin_folder') . '/holidays');
        }
    }
	
    /*     * ******************************************************************
      delete categorie
     * ****************************************************************** */

    function delete($id) {

        $membre = $this->md_commun->get_row('holidays',array('id'=>$id));

        if (membre) {
			
            $this->Holidays_model->delete($id);
            $this->session->set_flashdata('message', lang('message_deleted_categorie'));
        } else {
            $this->session->set_flashdata('error', lang('error_categorie_not_found'));
        }

        redirect($this->config->item('admin_folder') . '/holidays');
    }
	function upload($file)
    {
      $config['upload_path'] = 'uploads/avatars/';
      $config['allowed_types'] = 'gif|jpg|jpeg|png';
      $config['encrypt_name'] = true;
      $config['required'] = false;

      $this->load->library('upload', $config);
      $uploaded = $this->upload->do_upload($file);

      if ($uploaded) {
        $image = $this->upload->data();
        return $image['file_name'];
      }else
      return false;
    }
    function notfound() {
        $this->view('404');
    }


}
