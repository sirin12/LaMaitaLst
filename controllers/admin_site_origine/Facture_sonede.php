<?php
class Facture_sonede extends Admin_Controller {
	...
	function index() {
		...
		$data['facture_sonedes']=$this->md_commun->fetch('facture_sonede',array(),'asc',1000,0);
        $data['page_icon'] = 'icon-file-alt';

        $this->view('facture_sonedes', $data);
    }
	...
	function form_gare($id = false) {
        ...
        if ($this->form_validation->run() == false) {
			...
			redirect($this->config->item('admin_folder') . '/Facture_sonede/gares');
        }
    }
	...
	function form($id = false) {
		...
		$data['gares']=$this->md_commun->fetch('gares',array(),'asc',100,0);
		if($id){
			$facture_id=$this->md_commun->get_row('facture_sonede',array('id'=>$id));
			if (!$facture_id) {
                //page does not exist
                $this->session->set_flashdata('error', lang('error_page_not_found'));
                redirect($this->config->item('admin_folder') . '/Facture_sonede');
            }
			$data['id']=$id;
		...
		// Validate the form
        if ($this->form_validation->run() == false) {
            $this->view('facture_sonedes_form', $data);
        } else {
			$save = array();
			$save['id'] = $id;
			$save['gare'] = $this->input->post('gare');
			...
			$gare_id = $this->md_commun->save('facture_sonede',$save);
	...
	function recap() {
        $data['page_title'] = 'متابعة السنوية لإستهلاك الماء';
		$current_admin = $this->session->userdata('admin');
		$id_admin=$current_admin['id'];

		$data['gares']=$this->md_commun->fetch('gares',array(),'asc',100,0);
		$data['facture_sonede']=$this->md_commun->fetch('facture_sonede',array(),'asc',100,0);
		
        $data['page_icon'] = 'icon-file-alt';

        $this->view('recap_sonede', $data);
    }
