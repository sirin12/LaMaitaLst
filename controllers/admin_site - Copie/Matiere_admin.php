<?php
class Matiere_admin extends Admin_Controller {

    function __construct() {
        parent::__construct();

        $this->auth->check_access('Admin', true);
		$this->load->model(array('Class_model','Filieres_model','Matieres_model','Coefficient_model','Enseignants_model','Etudiants_model','Annees_model'));
    }
	...
	function index() {
        ...
        $data['coefficients'] = $this->Class_model->get_matiere_admin();
	...
	function notes($id) {
		...
		$data['notes']=$this->Etudiants_model->get_note_matiere_admin_etud('2020',$coiefficient_id->matiere_id,$coiefficient_id->class_id);
        //$data['coefficients'] = $this->Class_model->get_matiere_enseignant_notes($id_admin,$id);
        $data['page_icon'] = 'icon-file-alt';

        $this->view('admin_matiere_notes', $data);
    }
	...
	function form_note($coief,$id = false) {
		...
		$i=0;if($etudiants){foreach($etudiants as $etudiant){$i++;
			...
			$coefficient_id = $this->Etudiants_model->save_note_admin($save);
		}}
			$this->session->set_flashdata('message', lang('message_saved_categorie'));
			redirect($this->config->item('admin_folder') . '/Matiere_admin/notes/'.$coief);
	...
	function form_note_edit($coief,$id = false) {
		...
		$notes=$this->Etudiants_model->get_note_matiere_admin_etud('2020',$coiefficient_id->matiere_id,$coiefficient_id->class_id,$id_admin);
		...
		$i=0;if($notes){foreach($notes as $note){$i++;
			...
			if($id_note==$note->id){
				...
				$coefficient_id = $this->Etudiants_model->save_note_admin_edit($save);
			}
		}}
			$this->session->set_flashdata('message', lang('message_saved_categorie'));
			redirect($this->config->item('admin_folder') . '/Matiere_admin/notes/'.$coief);
	...
	function valider($id) {
        ...
        redirect($this->config->item('admin_folder') . '/Matiere_admin');
    }
